...
bool CachingVPExpander::expandVectorPredication(VPIntrinsic &VPI) {
  ...
  Strategy.EVLParamStrategy = VPLegalization::Legal;
  ...
  case VPLegalization::Convert:
    expandPredication(VPI);
    ++NumLoweredVPOps;
    break;
  }
  Strategy.OpStrategy = VPLegalization::Legal;

  assert(Strategy.shouldDoNothing() && "incomplete transformation");
