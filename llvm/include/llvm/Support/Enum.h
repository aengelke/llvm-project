...
template <typename T, unsigned NumStrs = 1> struct EnumStringDef {
  // Actual length name is irrelevant, this is not stored in the binary. Bump
  // if a larger maximum string name is needed.
  std::array<char[72], NumStrs> Names;
  T Value;
};
...
enumStringsStorageSize(const EnumStringDef<T, NumStrs> (&Entries)[N]) {
  ...
  for (unsigned i = 0; i != N; i++)
    for (unsigned j = 0; j != NumStrs; j++)
      Len += std::string_view(Entries[i].Names[j]).size();
  return Len;
}
} // namespace detail
