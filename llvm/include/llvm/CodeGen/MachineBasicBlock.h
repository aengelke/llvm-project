...
template <> struct GraphTraits<MachineBasicBlock *> {
  ...
  static int getNumber(MachineBasicBlock *BB) { assert(BB->getNumber() >= 0 && "negative block number"); return BB->getNumber(); }
};
...
template <> struct GraphTraits<const MachineBasicBlock *> {
  ...
  static int getNumber(const MachineBasicBlock *BB) { assert(BB->getNumber() >= 0 && "negative block number"); return BB->getNumber(); }
};
