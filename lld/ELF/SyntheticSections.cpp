...
CieRecord *EhFrameSection::addCie(EhSectionPiece &cie,
  ...
  if (!rec) {
    rec = make<CieRecord>();
    rec->cie = &cie;
    // If --eh-frame-hdr is given and the FDEs for the CIE use compact unwind
    // descriptors, emit them as inline entries in .eh_frame_hdr.
    rec->isCompact = ctx.arg.ehFrameHdr && isCompactUnwind(cie);
    if (rec->isCompact) {
      hasCompactUnwind = true;
      if (personality) {
...
void EhFrameHeader::finalizeContents() {
  ...
  if (!ehFrame->hasCompactUnwind) {
    // Compute size: 4-byte header + eh_frame_ptr + fde_count + FDE table.
    // Initially `large` is false; updateAllocSize may set it to true if
    // addresses exceed the 32-bit range, then call finalizeContents again.
  ...
  for (CieRecord *rec : ehFrame->getCieRecords()) {
    ...
    for (EhSectionPiece *fde : rec->fdes) {
      ...
      uint64_t addrRange =
          readCompactUnwindDescriptors(*fde, fdeEnc, rec->isCompact, descs);
      if (int64_t(pcRel + addrRange) > lastVARel)
        lastVARel = pcRel + addrRange;
      // newLarge |= !isInt<32>(pcRel); XXX
      if (!rec->isCompact) {
        assert(start == descs.size());
        ...
        descs.push_back({pcRel + addrRange, 1});
