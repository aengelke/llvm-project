...
template <> struct TypeMap<BasicBlock> {
  ...
  using CycleT = GenericCycle<SSAContext>;
...
template <> struct TypeMap<MachineBasicBlock> {
  ...
  using CycleT = GenericCycle<MachineSSAContext>;
...
template <class BT> class BlockFrequencyInfoImpl : BlockFrequencyInfoImplBase {
  ...
  using CycleT = typename bfi_detail::TypeMap<BT>::CycleT;
...
template <class BT> void BlockFrequencyInfoImpl<BT>::initializeLoops() {
  ...
  std::deque<std::pair<const CycleT *, LoopData *>> Q;
  for (const CycleT *C : CI->toplevel_cycles())
    Q.emplace_back(C, nullptr);
  while (!Q.empty()) {
    const CycleT *Cycle = Q.front().first;
    LoopData *Parent = Q.front().second;
    Q.pop_front();

    if (Cycle->isReducible()) {
      BlockNode Header = getNode(Cycle->getHeader());
    ...
    for (const CycleT *C : Cycle->children())
  ...
  for (size_t Index = 0; Index < RPOT.size(); ++Index) {
    ...
    const CycleT *Cycle = CI->getCycle(RPOT[Index]);
    while (Cycle && !Cycle->isReducible())
