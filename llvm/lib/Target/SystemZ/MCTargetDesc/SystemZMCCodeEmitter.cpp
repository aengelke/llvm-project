...
private:
  ...
  uint64_t getPCRelEncoding(const MCInst &MI, unsigned OpNum,
                            VectorWriter<MCFixup> &Fixups, unsigned Kind,
...
void SystemZMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  size_t FixupStartIdx = Fixups.size();
  ...
  assert(Fixups.size() <= FixupStartIdx + 2 &&
...
uint64_t SystemZMCCodeEmitter::getPCRelEncoding(const MCInst &MI,
                                                unsigned OpNum,
                                                VectorWriter<MCFixup> &Fixups,
