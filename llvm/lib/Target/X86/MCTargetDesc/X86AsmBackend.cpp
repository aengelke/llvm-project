...
protected:
  ...
  static unsigned PushInstrSize(MCRegister Reg) {
    switch (Reg.id()) {
    case X86::EBX:
    case X86::ECX:
    case X86::EDX:
    ...
    case X86::R15:
      return 2;
    default:
      llvm_unreachable("unexpected push register");
    }
  }
...
public:
  ...
  void generateCompactUnwindEncoding(MCDwarfFrameInfo &FI,
    ...
    if (!generateCompactUnwindEncodingImpl(FI, Ctxt)) {
      // Something failed. Clean up any descriptors we might have written.
      dbgs() << "Compact unwind encoding failed\n";
      FI.CompactUnwindDescriptors.clear();
    }
  }

private:
  bool generateCompactUnwindEncodingImpl(MCDwarfFrameInfo &FI,
                                         const MCContext *Ctxt) const;
};

bool ELFX86AsmBackend::generateCompactUnwindEncodingImpl(
  ...
  constexpr unsigned CU_NUM_SAVED_REGS = 6;
  ...
  struct CFIState {
    ...
    void print(const MCRegisterInfo &MRI, raw_ostream &os) const {
      os << "CFIState{" << CFAOff << "," << (IsRBP ? "rbp" : "rsp") << ","
         << int(NumRegs);
  ...
  auto FlushDesc = [&]() {
      ...
      if (!PrologueAdvances.empty()) {
        ...
        for (unsigned i = 1; i < DescState.NumRegs; i++) {
          if (PrologueAdvances[i - 1] != PushInstrSize(DescState.Regs[i])) {
            dbgs() << "FAIL: advance prologue for off " << i
                   << " size mismatch: " << PrologueAdvances[i - 1]
                   << " != " << PushInstrSize(DescState.Regs[i]) << "\n";
            return false;
          }
        }
    ...
    FI.CompactUnwindDescriptors.emplace_back(DescStart, Desc);
  ...
  auto Advance = [&]() {
    ...
    case ModeRSPPrologue:
      ...
      LLVM_FALLTHROUGH;
    case ModeRSPBody:
      dbgs() << "rspbody" << CurState.CFAOff << " " << LastState.CFAOff
             << CurState.NumRegs << "\n";
      if (!CurState.IsRBP && CurState.CFAOff < LastState.CFAOff) {
  ...
  for (const MCCFIInstruction &Inst : FI.Instructions) {
    ...
    if (Cur->getFragment() != Inst.getLabel()->getFragment() ||
      ...
      } else if (Cur->getFragment()->getNext() ==
                     Inst.getLabel()->getFragment() &&
                 Cur->getFragment()->getKind() == MCFragment::FT_Data) {
        assert(Cur->getFragment()->getVarSize() == 0);
        LastAdvance = Cur->getFragment()->getFixedSize() - Cur->getOffset() +
                      Inst.getLabel()->getOffset();
      } else {
        LastAdvance = 0;
      }
      LastState = CurState;
