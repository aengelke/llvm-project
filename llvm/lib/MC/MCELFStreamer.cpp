...
void MCELFStreamer::emitInstToData(const MCInst &Inst,
  ...
  if (Assembler.isBundlingEnabled()) {
    // Can't encode directly into fragment as code path depends on whether
    // fixups are present.
    ...
    if (isBundleLocked() && !Sec.isBundleGroupBeforeFirstInst()) {
      ...
      //
      // Apparently, this is not just an optimization: using a data fragment
      // here has correctness issues.
  ...
  MutableArrayRef<MCFixup> Fixups = DF->getFixups().slice(FixupStartIdx);
  for (auto &Fixup : Fixups) {
    fixSymbolsInTLSFixups(Fixup.getValue());
